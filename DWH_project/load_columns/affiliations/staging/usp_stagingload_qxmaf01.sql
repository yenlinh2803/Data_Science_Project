...
CREATE OR REPLACE PROCEDURE staging.usp_stagingload_qxmaf01(
...
BEGIN
	...
	/* DETERMIZE LOAD RANGE */
		SELECT max(updated_at) INTO v_currentBatch.eor_ts FROM src_qux_main.affiliations;
		
		IF (v_currentBatch.eor_ts <= v_currentBatch.bor_ts) THEN
			RAISE NOTICE 'No change. Cancel!';
			RAISE LOG 'staging.usp_stagingload_qxmaf01: No change detected. Cancel!';
			RETURN;
		END IF;
	...
	IF(v_currentBatch.load_type_id = 1) THEN
		INSERT INTO staging.qxmaf01_quoinex_affiliations_t(
			-- Controled columns
                ctrl_batch_id, 
        ...
        id ,
			...
			SELECT 
			 ...
			 -- Payload
				...
				affiliate_credited_at ,
				from_exchange
			FROM src_qux_main.affiliations
