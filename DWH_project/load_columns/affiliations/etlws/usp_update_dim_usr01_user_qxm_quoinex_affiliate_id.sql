...
CREATE OR REPLACE PROCEDURE etlws.usp_update_dim_usr01_user_qxm_quoinex_affiliate_id(p_batch_id int4)
 LANGUAGE plpgsql
AS $function$ 
DECLARE
	-- CONST
	c_DES_OBJCODE varchar(10):='USR01';
	c_SRC_OBJCODE varchar(10):='QXMAF01';
	c_FLOW varchar(150):='usp_update_dim_usr01_user_qxm_quoinex_affiliate_id';
	-- BATCH
	v_flow_load_id int4;
	v_loadType_id int2;
	v_partitionKey int2;
...
--============ DETECT EXISTING ROWS ===============	
	...
	AS
	SELECT stg.referred_id as id
	FROM staging.qxmaf01_quoinex_affiliations_t AS stg 
		INNER JOIN edw_dim.usr01_users_t AS edw ON stg.referred_id = edw.id
...
--=============================================== UPDATE ===========================================
	...
	SET ctrl_batch_id  			= staging.qxmaf01_quoinex_affiliations_t.ctrl_batch_id, 
		ctrl_last_load_type_fl  = 'U', 
		ctrl_source_system_id  	= 2, 
		ctrl_last_update_ts  	= clock_timestamp(), 
		affiliate_id = staging.qxmaf01_quoinex_affiliations_t.affiliate_id 
	FROM staging.qxmaf01_quoinex_affiliations_t
	WHERE edw_dim.usr01_users_t.id = staging.qxmaf01_quoinex_affiliations_t.referred_id
		AND staging.qxmaf01_quoinex_affiliations_t.ctrl_batch_id 		= p_batch_id
		AND staging.qxmaf01_quoinex_affiliations_t.batch_partition_key = p_batch_id%4
		AND staging.qxmaf01_quoinex_affiliations_t.referred_id IN(SELECT id FROM temp_existing_id);
...
--=============================================== INSERT ===========================================
	...
	INSERT INTO edw_dim.usr01_users_t
		...
		SELECT  p_batch_id AS batch_id, 'I', 2, clock_timestamp(),
						referred_id, affiliate_id
		FROM staging.qxmaf01_quoinex_affiliations_t
		WHERE staging.qxmaf01_quoinex_affiliations_t.referred_id NOT IN(SELECT id FROM temp_existing_id)
			AND staging.qxmaf01_quoinex_affiliations_t.ctrl_batch_id 		= p_batch_id
			AND staging.qxmaf01_quoinex_affiliations_t.batch_partition_key = p_batch_id%4;
...
ELSE
	...
	INSERT INTO edw_dim.usr01_users_t
		...
		SELECT  p_batch_id AS batch_id, 'I', 2, clock_timestamp(), 
						referred_id, affiliate_id
		FROM staging.qxmaf01_quoinex_affiliations_t
		WHERE staging.qxmaf01_quoinex_affiliations_t.ctrl_batch_id =p_batch_id
			AND staging.qxmaf01_quoinex_affiliations_t.batch_partition_key = p_batch_id%4;
