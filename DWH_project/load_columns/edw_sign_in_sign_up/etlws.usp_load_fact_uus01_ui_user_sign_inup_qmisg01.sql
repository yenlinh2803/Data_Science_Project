...
CREATE OR REPLACE PROCEDURE etlws.usp_load_fact_uus01_ui_user_sign_inup_qmisg01(p_batch_id integer)
 LANGUAGE plpgsql
AS $procedure$ 
DECLARE
	-- CONST
	c_DES_OBJCODE varchar(10):=upper('uus01');
	c_SRC_OBJCODE varchar(10):= upper('qmisg01');
	c_FLOW varchar(150):='usp_load_fact_uus01_ui_user_sign_inup_qmisg01';
	-- BATCH
	v_flow_load_id int4;
	v_loadType_id int2;
	v_partitionKey int2;
...
--	FROM staging.qmisg01_liq_mobile_ios_sign_in_t stg
...
--=============================================== INSERT ===========================================
  ...
  -- Control columns
    ...
    SELECT  
		...
		ano.id,
  -- Event info
		1 AS action_id,/* action_id: - 3: sign_up - 1: sign_in - 4: sign_up_submit */
		stg."event",
		stg.event_text,
		1 AS ui_type_id, /* ui_type_id: - 0: web - 1: mobile  */
  -- context info
   -- app info
    NULL context_app_build           ,
