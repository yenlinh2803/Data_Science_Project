...
CREATE OR REPLACE PROCEDURE etlws.usp_load_fact_uus01_ui_user_sign_inup_qmasg01(p_batch_id integer)
 LANGUAGE plpgsql
AS $procedure$ 
DECLARE
	-- CONST
	c_DES_OBJCODE varchar(10):=upper('uus01');
	c_SRC_OBJCODE varchar(10):= upper('qmasg01');
	c_FLOW varchar(150):='usp_load_fact_uus01_ui_user_sign_inup_qmasg01';
	-- BATCH
	v_flow_load_id int4;
	v_loadType_id int2;
	v_partitionKey int2;
...
--	FROM staging.qmasg01_liq_mobile_android_sign_in_t stg
...
----============ DETECT EXISTING ROWS ===============	
	...
	;
	GET DIAGNOSTICS v_sta_eff_count = ROW_COUNT;
	PERFORM etlctrl.uf_add_internalflow_etl_log(v_flow_load_id,' -- temp_exists_id row: '|| v_sta_eff_count);
	COMMIT;
	
	insert into edw_fact.ano01_ui_anonymous_t(anonymous_id)
		SELECT distinct tmp.anonymous_id
		FROM temp_exists_id tmp
		where tmp.isano is null ;
