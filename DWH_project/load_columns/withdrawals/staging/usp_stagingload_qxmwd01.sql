...
CREATE OR REPLACE PROCEDURE staging.usp_stagingload_qxmcw03(
...
BEGIN
	...
	/* DETERMIZE LOAD RANGE */
		SELECT max(updated_at) INTO v_currentBatch.eor_ts FROM src_qux_main.crypto_withdrawals;
		
		IF (v_currentBatch.eor_ts <= v_currentBatch.bor_ts) THEN
			RAISE NOTICE 'No change. Cancel!';
			RAISE LOG 'staging.usp_stagingload_qxmcw03: No change detected. Cancel!';
			RETURN;
		END IF;
	...
	IF(v_currentBatch.load_type_id = 1) THEN
		INSERT INTO staging.qxmcw03_quoinex_crypto_withdrawals_t(
			-- Controled columns
                ctrl_batch_id, 
        ...
        id  ,
			...
			SELECT 
			 ...
			 -- Payload
				...
				memo_type ,
				memo_value  
			FROM src_qux_main.crypto_withdrawals
