-- SET ROLE data_team;
--CALL staging.usp_stagingload_qmisi03();
CREATE OR REPLACE PROCEDURE staging.usp_stagingload_qmisi03(p_is_overlap_load boolean DEFAULT false, p_overlap_interval interval DEFAULT NULL::interval, p_overlap_from_bor timestamp without time zone DEFAULT NULL::timestamp without time zone)
 LANGUAGE plpgsql
AS $procedure$ 
DECLARE
	-- CONST
	c_OBJCODE varchar(10):= upper('qmisi03');
...
BEGIN
	...
	/* DETERMIZE LOAD  RANGE */
		SELECT max(received_at) INTO v_currentBatch.eor_ts FROM src_seg_liq_mobile.sign_up;
	...
	IF(v_currentBatch.load_type_id = 1) THEN
		...
		INSERT INTO staging.qmisi03_liq_mobile_ios_sign_up_t(
		-- Control
			ctrl_batch_id,
			batch_partition_key,
...
--      CONTENT
			...
			context_device_ad_tracking_enabled,
			context_device_advertising_id 
		FROM src_seg_liq_mobile.sign_up
